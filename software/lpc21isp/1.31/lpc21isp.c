...
Change-History:
 ...
 1.15  2004-09-27  Temporary improvement by Cyril Holweck:
                   Removed test (data echoed = data transmited) on the 
...
******************************************************************************/
#define __LINUX__
#define VERSION_STR	"1.31"

/* LPC_RAMSTART, LPC_RAMBASE
 *
 * Used in PhilipsDownload() to decide wether to Flash code or just place in in RAM
...
#define LPC_FLASHMASK  0xFFC00000 /* 22 bits = 4 MB */
    ...
    #define COMPILE_FOR_MACOS
    #define COMPILED_FOR ("MacOS")
#endif
...
#define PHILIPS_ARM 0
#define ANALOG_DEVICES_ARM 1
...
#if !defined COMPILE_FOR_LPC21
    int micro;                      /**< The type of micro that will be
                                         * programmed.                           */
    int debug_level;
    int			TerminalAfterUpload;
    int			TerminalOnly;
    int			DetectOnly;
...
{//id, product, flash, ram, sectors, maxcopysize, sectable
  { 0, 0, 0 },  /* unknown */
...
int baudCheck(baudRate)
{
	switch(baudRate) {
        ...
        default:
			printf("Unknown baudrate %u, but it anyway\n",baudRate);
			return baudRate;
    }
}	
...
static void OpenSerialPort(ISP_ENVIRONMENT *IspEnvironment)
{
    ...
    DebugPrintf( 3, "X COM-Port %s opened...\n", IspEnvironment->serial_port);
	...
	cfsetspeed(&IspEnvironment->newtio,baudCheck(atol(IspEnvironment->baud_rate)));
...
static void ControlModemLines(ISP_ENVIRONMENT *IspEnvironment, unsigned char DTR, unsigned char RTS)
{
    ...
    {
        debug( 3, "ioctl get ok, status = %X\n",status);
    }
    else
    {
        debug( 1, "ioctl get failed\n");
    }
    ...
    {
        debug( 3, "ioctl set ok, status = %X\n",status);
    }
    else
    {
        debug( 1, "ioctl set failed\n");
    }
...
static void LoadFile(ISP_ENVIRONMENT *IspEnvironment)
{
    ...
    if(IspEnvironment->FormatHex != 0)
    {
        ...
        debug( 1, "Converting file %s to binary format...\n", IspEnvironment->input_file);
        ...
        while(Pos < FileLength)
        {
            ...
            if(FileContent[Pos] != ':')
            {
                debug( 1, "Missing start of record (':') wrong byte %c / %02X\n", FileContent[Pos], FileContent[Pos]);
                exit(1);
            }
            ...
            else if(RecordType == 0x04)     // 04 - Extended linear address record, used by IAR
            {
				...
				else
				{
					...
					{
						debug(1, "New Extended Linear Address Record [04] out of memory range\n");
						debug(1, "Current Memory starts at: 0x%08X, new Address is: 0x%08X",
        ...
        debug( 2, "\tconverted to binary format...\n");
    ...
    if (IspEnvironment->BinaryLength % 4 != 0)
    {
        ...
        debug( 2, "Warning:  data not aligned to 32 bits, padded (length was %lX, now %lX)\n", IspEnvironment->BinaryLength, NewBinaryLength);
...
static void AnalogDevicesSync(ISP_ENVIRONMENT *IspEnvironment)
{
    ...
    for(sync_attempts = 0; sync_attempts < 5; sync_attempts++)
    {
        ...
        {
            debug( 3, "No (or incomplete) answer on sync attempt\n");
        }

        ClearSerialPortBuffers(IspEnvironment);
    }

    debug( 1, "No (or unacceptable) answer on sync attempt\n");
    exit(4);
}
                                     ...
                                     const AD_PACKET * packet)
{
    ...
    do {
        ...
        if( ReceiveComPortBlockComplete( IspEnvironment, &response, 1, 500) == 0)
        {
            ...
            {
                debug( 3, "Unexpected response to packet (%x)\n", (int)response);
            }
            debug( 2, "*");
        }
    } while( retry < 3);

    debug( 1, "Send packet failed\n");
    exit( -1);
}
...
static int AnalogDevicesDownload(ISP_ENVIRONMENT *IspEnvironment)
{
    ...
    {
        debug( 2, "Note:  Flash remapped 0x80000 to 0.\n");
...
int PhilipsDownload(ISP_ENVIRONMENT *IspEnvironment)
{
    ...
    if(!IspEnvironment->DetectOnly)
    {
        ...
        debug( 3, "Position 0x14 patched: ivt_CRC = 0x%08lX\n", ivt_CRC);
    }
    ...
    for(nQuestionMarks = found = 0; !found && nQuestionMarks < 100; nQuestionMarks++)
    {
        ...
        if(strcmp(strippedAnswer, "Bootloader\r\n") == 0 && IspEnvironment->TerminalOnly == 0)
        {
            ...
            debug( 2, "Entering ISP; re-synchronizing (watchdog = %ld seconds)\n", WatchDogSeconds);
            sprintf(temp, "T %lu\r\n", ticks);
    ...
    {
        debug( 1, "No answer on 'Synchronized'\n");
        return(4);
    }

    debug( 3, "Synchronized 1\n");

    debug( 3, "Setting oscillator\n");
    ...
    {
        debug( 1, "No answer on Oscillator-Command\n");
        return(4);
    }
    ...
    if(strncmp(Answer, "K\r\n", 3) != 0)
    {
        debug( 1, "no answer on Read Boot Code Version\n");
        return(4);
    }
    ...
    if(strncmp(Answer, "J\r\n", 3) != 0)
    {
        debug( 1, "no answer on Read Part Id\n");
        return(4);
    }
    ...
    *endPtr = '\0'; /* delete \r\n */
    ...
    if (IspEnvironment->DetectedDevice == 0){
        ...
        debug( 2, "LPC%d, %d kiB ROM / %d kiB SRAM",
    ...
    for(SectorStart = Sector = 0; ; SectorStart += LPCtypes[IspEnvironment->DetectedDevice].SectorTable[Sector++])
    {
        ...
        {
            debug( 1, "Program too large; running out of Flash sectors.\n");
            return(4);
        }

        debug(2, "Sector %ld: ", Sector);
        fflush(stdout);
		...
		if(IspEnvironment->BinaryOffset < LPC_RAMSTART) // Skip Erase when running from RAM
		{
        	...
        	{
        	    debug( 1, "Wrong answer on Prepare-Command (1) (Sector %ld)\n", Sector);
        	    PhilipsOutputErrorMessage(GetErrorNumber(Answer));
        	    return(4);
        	}
        	...
        	if (!SendAndVerify(IspEnvironment, tmpString, Answer, sizeof Answer))
        	{
        	    debug( 1, "Wrong answer on Erase-Command (Sector %ld)\n", Sector);
        	    PhilipsOutputErrorMessage(GetErrorNumber(Answer));
        	    return(4);
        	}
			if(DebugNeed(2)) printf(".");
        ...
        for(SectorOffset = 0; SectorOffset < SectorLength; SectorOffset += SectorChunk)
        {
			if (SectorOffset > 0)
			{
                ...
                debug(2, "|");  /* means: partial segment copied */
                fflush(stdout);
			}
            ...
            if (!SendAndVerify(IspEnvironment, tmpString, Answer, sizeof Answer))
            {
                debug( 1, "Wrong answer on Write-Command\n");
                PhilipsOutputErrorMessage(GetErrorNumber(Answer));
                return(4);
            }
			if(DebugNeed(2)) printf(".");
            ...
            for(Pos = SectorStart + SectorOffset; (Pos < SectorStart + SectorOffset + CopyLength) && (Pos < IspEnvironment->BinaryLength); Pos += (45 * 4))
            {
                for(Block = 0; Block < 4; Block++)  // Each block 45 bytes
                {
                    ...
                    {
                        debug( 1, "Error on writing data (1)\n");
                        return(4);
                    }
                    ...
                    if(Line == 20)
                    {
                        ...
                        {
                            debug( 1, "Error on writing block_CRC (1)\n");
                            return(4);
                        }
            ...
            if(Line != 0)
            {
                ...
                if(strcmp(Answer, tmpString) != 0)
                {
                    debug( 1, "Error on writing block_CRC (2)\n");
                    return(4);
                }
            }
			...
			if(IspEnvironment->BinaryOffset < LPC_RAMSTART)
			{
            	...
            	if (!SendAndVerify(IspEnvironment, tmpString, Answer, sizeof Answer))
            	{
            	    debug( 1, "Wrong answer on Prepare-Command (2) (Sector %ld)\n", Sector);
            	    PhilipsOutputErrorMessage(GetErrorNumber(Answer));
            	    return(4);
            	}
    ...
    debug( 2, "Download Finished... taking %d seconds\n", tDoneUpload - tStartUpload);

    if (WaitForWatchDog)
    {
        debug( 2, "Wait for restart, in %d seconds from now\n", WatchDogSeconds - (tDoneUpload - tStartUpload));
    }
    else
    {
        debug( 2, "Now launching the brand new code\n" );
        fflush(stdout);
        ...
        { // Skip response on G command - show response on Terminal instead
        	...
        	{
        	    debug( 2, "Failed to run the new downloaded code: ");
        	    PhilipsOutputErrorMessage(GetErrorNumber(Answer));
        	    return(4);
        	}
		}
...
static void Terminal( ISP_ENVIRONMENT *IspEnvironment)
{
    ...
    do
    {
        ...
        if ( kbhit() )
        {
            ...
            if(ch == 0x1b)
            {
				printf("\nq 01234?");fflush(stdout);
				ch=getch();
				switch(ch){
				case 'q':break;
				case '0':case '1':case '2':case '3':case '4':
					ControlModemLines(IspEnvironment, (ch&1)?1:0,(ch&2)?1:0);continue;
				}
                break;
            }
...
//#if !defined COMPILE_FOR_LPC21
    switch(ErrorNumber)
    {
        ...
        case   2:
            txt = "SRC_ADDR_ERROR: Source address is not on word boundary.";
            break;
        case   3:
            txt = "DST_ADDR_ERROR: Destination address is not on a correct boundary.";
            break;

        case   4:
            txt = "SRC_ADDR_NOT_MAPPED: Source address is not mapped in the memory map.\n"
                                 " Count value is taken into consideration where applicable.";
            break;

        case   5:
            txt = "DST_ADDR_NOT_MAPPED: Destination address is not mapped in the memory map.\n"
                                 " Count value is taken into consideration where applicable.";
            break;

        case   6:
            txt = "COUNT_ERROR: Byte count is not multiple of 4 or is not a permitted value.";
            break;

        case   7:
            txt = "INVALID_SECTOR: Sector number is invalid or end sector number is\n"
                          "  greater than start sector number.";
            break;

        case   8:
            txt = "SECTOR_NOT_BLANK";
            break;

        case   9:
            txt = "SECTOR_NOT_PREPARED_FOR_WRITE_OPERATION:\n"
             "Command to prepare sector for write operation was not executed.";
            break;

        case  10:
            txt = "COMPARE_ERROR: Source and destination data not equal.";
            break;

        case  11:
            txt = "BUSY: Flash programming hardware interface is busy.";
            break;

        case  12:
            txt = "PARAM_ERROR: Insufficient number of parameters or invalid parameter.";
            break;

        case  13:
            txt = "ADDR_ERROR: Address is not on word boundary.";
            break;

        case  14:
            txt = "ADDR_NOT_MAPPED: Address is not mapped in the memory map.\n"
        ...
        case  16:
            txt = "INVALID_CODE: Unlock code is invalid.";
            break;

        case  17:
            txt = "INVALID_BAUD_RATE: Invalid baud rate setting.";
            break;

        case  18:
            txt = "INVALID_STOP_BIT: Invalid stop bit setting.";
            break;

        case 255:                                                                                            break;
        default:
            txt = "unknown error %u", ErrorNumber;
            break;
    }
	debug(1,"%s",txt);
...
int main(int argc, char *argv[])
{
	...
	oAdd("-h --hex","","0",(p_t)g_chk,&IspEnvironment.FormatHex);
	oAdd("-b --bin","","1",(p_t)g_chk,&IspEnvironment.FormatHex);
	oAdd("-e --elf","","2",(p_t)g_chk,&IspEnvironment.FormatHex);

	oAdd("-t --term","Terminal after load","1",(p_t)g_bool,&IspEnvironment.TerminalAfterUpload);
	oAdd("-o --termonly","No load only terminal","0",(p_t)g_bool,&IspEnvironment.TerminalOnly);
	oAdd("-d --detect","Only chip type","0",(p_t)g_bool,&IspEnvironment.DetectOnly);
	oAdd("-c --control","Use controls reset=DTR boot=DSR","1",(p_t)g_bool,&IspEnvironment.DetectOnly);
	
	oAdd("-p --philips","Philips ARM","0",(p_t)g_chk,&IspEnvironment.micro);
	oAdd("-a --adm","Analog Devices ARM","1",(p_t)g_chk,&IspEnvironment.micro);

	oAdd("-r# --baud","Baud rate of com port","115200",(p_t)g_text,&IspEnvironment.baud_rate);
	oAdd("-s# --serial","Serial port to use","/dev/tty.usbserial0",(p_t)g_text,&IspEnvironment.serial_port);
	oAdd("-x# --xtal","Oscillator frequency","14746",(p_t)g_text,&IspEnvironment.StringOscillator);
	oAdd("-f# --file","File to upload","code.hex",(p_t)g_text,&IspEnvironment.input_file);
...
//    ReadArguments( &IspEnvironment, argc, argv);
	debug( 1, "lpc21isp version " VERSION_STR "\n");
