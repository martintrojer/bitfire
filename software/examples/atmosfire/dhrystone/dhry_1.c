...
//  REG   int             Number_Of_Runs;
...
#ifdef BITFIRE
  {
    ...
    gltxt_printtext("dhry",&p,&glcol_yellow);
  }
  ...
  RCCU_PCLKConfig(RCCU_RCLK_8);				  Begin_Time = TIM_CounterValue(TIM1);
	// APB2 slow peripherals
...
//  EIC_IRQChannelConfig(T1TIMI_IRQChannel, ENABLE );
  ...
  Begin_Time = TIM_CounterValue(TIM1);
...
#ifdef ATMOSFIRE
  End_Time = TIM_CounterValue(TIM1);
#endif
...
#ifdef ATMOSFIRE
  User_Time = of_cnt*0xffff + End_Time; // - Begin_Time;
#else
...
#ifdef BITFIRE
    gl_point p = {0,0};
    gltxt_printtext("err",&p,&glcol_yellow);
#endif
  ...
  else
  {
    ...
    Dhrystones_Per_Second = 
      (float) Number_Of_Runs / (((float) User_Time)/((float) CLOCKS_PER_SEC));
    gltxt_printhex(Dhrystones_Per_Second,&glcol_yellow,1);
#endif
